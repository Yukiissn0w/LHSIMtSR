...
class Pix2PixHDModel_condImg_imp(BaseModel):
    def __init__(self, opt):
        super(Pix2PixHDModel_condImg_imp, self).__init__(opt)
    ...
    def name(self):
        return 'Pix2PixHDModel_condImg_imp'
    ...
    def forward(self, label, inst, image, feat, mask_in, mask_out, infer=False, classifier=None, imp_label=None):
        ...
        # predicted = classifier(util.tensor2im(fake_image.cpu().data[0]))
        # loss_G_imp = self.criterionIMP(predicted, imp_label)
        loss_G_imp = self.criterionIMP(predicted, imp_label.squeeze()) * self.opt.lambda_imp
