...
class SegmentationDataset(BaseDataset):
    ...
    def initialize(self, opt): # config=DEFAULT_CONFIG):
        ...
        self.A_paths = sorted(make_dataset(self.dir_A)) #dir_Aをos.walkしファイル名をappendし保存
    ...
    def get_raw_inputs(self, index):
        ...
        # imp label
        # print("A_path:{},{}".format(A_path, A_path.split('/')[-1].split('_')[2]))
        # input()
        if int(A_path.split('/')[-1].split('_')[2])>46: raw_inputs['imp_label']=1
        elif int(A_path.split('/')[-1].split('_')[2])<35: raw_inputs['imp_label']=0
        else:
            print('error:segmentation_imp_dataset.py line72 -> get_raw_inputs : {}'.format(A_path))
            error_stop = input()
            return error_stop
    ...
    def preprocess_cropping(self, raw_inputs, outputs, params):
        ...
        # ch 
        # LongTensorはcriterionIMPで
        # FloatTensorはobj_feat = impression_view * obj_feat1 + (1 - impression_view) * obj_feat0で
        outputs['imp_label_L'] = torch.LongTensor([raw_inputs['imp_label']])
        outputs['imp_label_F'] = torch.FloatTensor([raw_inputs['imp_label']])
        return outputs
