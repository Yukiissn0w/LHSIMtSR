...
class JointInference():
    def __init__(self, joint_opt):
        ...
        self.opt_imggen.gpu_ids = joint_opt.gpu_ids
    ...
    def sample_bbox(self, bbox_originals, min_box_size, random=False):
        candidate_list = []
        ...
        if not random and len(candidate_list) > 0:
            ...
            # Random sample
            # print("bbox_originals:{}, \ntype:{}".format(bbox_originals, type(bbox_originals)))
            # print("else:{}".format(np.random.choice(bbox_originals)))
            #ch
    ...
    def gen_image(self, bbox_sampled, img_original, label_generated, opt, imp_label):
        # crop canvas
        ...
        img_generated = self.G_mask2img.inference(
            ...
            Variable(input_dict['mask_in'], volatile=True),
            Variable(input_dict['mask_out'], volatile=True),
            Variable(imp_label, volatile=True)
        )
        # paste canvas
